...
ON transactions(member_id, event_id, campaign_id, contributor_type);`,
  ...
  {
    id: '041_org_auto_archive_not_null_repair',
    sql: `-- Backfill auto-archive organization settings for older rows
UPDATE organization
SET
...
WHERE
  ...
  {
    id: '042_email_outbox_dues_reminder',
    sql: `-- Allow DUES_REMINDER rows in email outbox
