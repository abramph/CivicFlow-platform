...
function unestra_pricing_faq_schema() {
	...
	$faqs = [
		...
		[ 'Is Stripe the only payment method organizations can use?', 'No. Stripe provides Unestra\'s integrated online checkout, but organizations can continue accepting approved external methods such as Zelle, Cash App, cash, checks, payroll deductions, or other configured options. External payments occur outside Unestra and may need to be reported, entered, or verified by authorized organization staff.' ],
		[ 'Does Unestra connect directly to Zelle or Cash App?', 'Not currently. Zelle and Cash App payments are completed outside Unestra. Where enabled, members or staff can document the payment in Unestra so authorized staff can review it and update the appropriate record.' ],
		[ 'Are externally reported payments automatically approved?', 'No. A reported external payment is not automatically treated as confirmed. The organization\'s authorized staff may need to review the payment details or receipt before updating dues, contribution, or payment records.' ],
		[ 'Can organizations decide which payment methods to accept?', 'Yes. Each organization determines which external payment methods it accepts and communicates its payment instructions to members. Available Unestra reporting and recording options depend on the organization\'s configuration.' ],
