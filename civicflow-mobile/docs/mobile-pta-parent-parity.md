...
- **`mobile-api.ts`** — new typed functions for every route above, plus three `*ForIdentity()` helpers (`getAnnouncementsForIdentity`, `markAnnouncementReadForIdentity`, `getEventsForIdentity`) that pick the conventional or PTA route based on `hasMemberIdentity`, so screens don't each re-derive the branch.
