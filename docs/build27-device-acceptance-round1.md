...
- **Honest limits**, unchanged from the matrix: invited-but-not-responded is not representable (no invitation model); capacity/waitlist exist only for volunteer shifts (already fully served on their own surfaces).
- **Follow-up (reviewer-required)**: the meetings-respondents-on-web-only limitation was rejected and closed — mobile now has the read-only `/admin-meetings/[meetingId]` planning screen (new `manageMeetings`-gated API; shared `AdminRsvpSection` with the event detail; loading/empty/error/pull-to-refresh; org-tagged state), and the dashboard's meeting rows open it. The web respondent-privacy review confirmed ordinary members/parents cannot reach any respondent table server-side (MEMBER role holds zero staff permissions — regression-pinned; parents hold no membership), so no web correction was needed.
