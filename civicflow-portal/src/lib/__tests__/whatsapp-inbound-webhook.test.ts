...
// Signature verification reuses the SMS account auth token — no separate
// WhatsApp credential exists. No credentials configured in the database
// here, so it resolves via the env-var fallback, same as the SMS webhook test.
...
const URL = "https://civicflow-portal-iule6.ondigitalocean.app/api/webhooks/twilio/whatsapp/inbound";
...
describe("Twilio WhatsApp inbound webhook", () => {
  ...
  it("rejects a request with no signature header", async () => {
    const request = new Request(URL, {
      ...
      body: new URLSearchParams({ From: "whatsapp:+15551234567", Body: "STOP" }).toString(),
    });
  ...
  it("rejects a request with an invalid signature", async () => {
    const request = makeRequest({ From: "whatsapp:+15551234567", Body: "STOP" }, { signature: "bogus-signature" });
    const response = await POST(request);
    expect(response.status).toBe(403);
    expect(updateManyOrgMember).not.toHaveBeenCalled();
  });

  it("strips the whatsapp: prefix before matching and opts a member out on a verified STOP keyword", async () => {
    queryRawOrgMember.mockResolvedValueOnce([
      { id: "member-1", organizationId: "org-1" },
      { id: "member-2", organizationId: "org-2" },
    ]);
    const request = makeRequest({ From: "whatsapp:+15551234567", Body: "STOP" });
    const response = await POST(request);
    expect(response.status).toBe(200);
    expect(updateManyOrgMember).toHaveBeenCalledWith({
      where: { id: { in: ["member-1", "member-2"] } },
      data: { whatsappEnabled: false, whatsappOptedOutAt: expect.any(Date), whatsappOptInStatus: "OPTED_OUT" },
    });
    expect(createAuditEvent).toHaveBeenCalledWith(
      expect.objectContaining({ organizationId: "org-1", action: "whatsapp_consent.opt_out", entityId: "member-1" })
    );
    expect(createAuditEvent).toHaveBeenCalledWith(
      expect.objectContaining({ organizationId: "org-2", action: "whatsapp_consent.opt_out", entityId: "member-2" })
    );
  });

  it("looks up members by digits-only whatsappPhoneNumber (full and last-10)", async () => {
    const request = makeRequest({ From: "whatsapp:+15551234567", Body: "STOP" });
    await POST(request);
  ...
  it("treats other STOP-family keywords the same way", async () => {
    const request = makeRequest({ From: "whatsapp:+15551234567", Body: "unsubscribe" });
    const response = await POST(request);
    expect(response.status).toBe(200);
    expect(updateManyOrgMember).toHaveBeenCalledWith({
  ...
  it("opts a member back in on a verified START keyword, clearing whatsappOptedOutAt and setting whatsappLastConfirmedAt", async () => {
    const request = makeRequest({ From: "whatsapp:+15551234567", Body: "START" });
    const response = await POST(request);
    expect(response.status).toBe(200);
    expect(updateManyOrgMember).toHaveBeenCalledWith({
      where: { id: { in: ["member-1"] } },
      data: {
        ...
        whatsappLastConfirmedAt: expect.any(Date),
      },
    });
    expect(createAuditEvent).toHaveBeenCalledWith(
      expect.objectContaining({ organizationId: "org-1", action: "whatsapp_consent.opt_in", entityId: "member-1" })
    );
  });

  it("replies with support info on a verified HELP keyword, without touching consent state", async () => {
    const request = makeRequest({ From: "whatsapp:+15551234567", Body: "HELP" });
    const response = await POST(request);
    expect(response.status).toBe(200);
  ...
  it("does nothing when no member matches the WhatsApp phone number", async () => {
    queryRawOrgMember.mockResolvedValueOnce([]);
    const request = makeRequest({ From: "whatsapp:+15551234567", Body: "STOP" });
    const response = await POST(request);
    expect(response.status).toBe(200);
