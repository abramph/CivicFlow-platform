...
export async function claimPtaVolunteerSlot(organizationId: string, slotId: string, householdAdultId: string, actorUserId: string, actorEmail?: string | null) {
  ...
  const claim = await prisma.ptaVolunteerSlot.updateMany({
...
/** Releases the claimed seat back to the slot — the mirror-image atomic decrement of claimPtaVolunteerSlot(). */
export async function cancelPtaVolunteerSignup(organizationId: string, slotId: string, householdAdultId: string, actorUserId: string, actorEmail?: string | null) {
  ...
  const updated = await prisma.ptaVolunteerSignup.update({ where: { id: signup.id }, data: { status: "CANCELLED", cancelledAt: new Date() } });
  await prisma.ptaVolunteerSlot.updateMany({ where: { id: slotId, claimedCount: { gt: 0 } }, data: { claimedCount: { decrement: 1 } } });
