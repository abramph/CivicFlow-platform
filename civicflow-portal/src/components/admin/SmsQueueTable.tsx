...
function QueueRow({ message }: { message: QueueMessage }) {
  ...
  async function act(action: "retry" | "cancel") {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/sms/messages/${message.id}/${action}`, { method: "POST" });
    ...
    <tr className="border-t border-slate-100">
      ...
      <td className="px-4 py-3">
        <div className="flex flex-wrap gap-2">
          ...
          {message.status === "QUEUED" || message.status === "RETRYING" ? (
            <button
              type="button"
              disabled={pending !== null}
              onClick={() => act("cancel")}
...
export function SmsQueueTable({ messages }: { messages: QueueMessage[] }) {
  return (
