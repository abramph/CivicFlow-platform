...
/**
 * Public, no-login-required "request account deletion" page. Mirrors
 * ForgotPasswordForm's shape exactly (same always-succeed response from the
 * API so this page can never be used to probe which emails are registered).
 * This is the entry point for mobile (opened via the system browser, which
 * doesn't share the app's session) and for anyone who no longer has app
 * access at all.
 */
    ...
    return (
      <div className="w-full max-w-md rounded-xl border border-slate-200 bg-white p-6 shadow-sm text-center">
        ...
        <p className="mt-2 text-sm text-slate-600">
          If <span className="font-medium text-slate-800">{email}</span> is registered, we sent a confirmation
  ...
  return (
    <div className="w-full max-w-md rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <p className="mt-1 text-sm text-slate-600">
        Enter your account email and we&apos;ll send you a link to confirm permanent deletion.
      </p>
      <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
        This deletes your personal login and profile only. It does not delete any organization you belong to, or
        its records.
      </div>
