...
export default async function MemberDuesPage({ searchParams }: { searchParams: Promise<{ org?: string }> }) {
  ...
  const payableMethods = filterPayableMethods(paymentMethods);
  ...
  return (
    <main className="mx-auto max-w-2xl space-y-6 p-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6">
        ...
        <Link href="/m/make-payment" className="mt-2 block text-sm font-medium text-emerald-700 hover:underline">
          Pay dues in advance, or contribute to a campaign or event →
        </Link>
      </div>

      <PayableMethodsList methods={payableMethods} />
