...
export default async function AttendanceAuditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!meeting) {
    return (
      <main className="space-y-6">
        <PageHeader title="Meeting not found" actions={[{ href: "/meetings", label: "Back to Meetings" }]} />
      </main>
    );
  }

  const sessions = await prisma.meetingAttendanceSession.findMany({ where: { organizationId, meetingId: id }, select: { id: true } });
  const records = await prisma.attendanceRecord.findMany({ where: { organizationId, meetingId: id }, select: { id: true } });
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        title={`${meeting.title} — Attendance Audit History`}
        actions={[{ href: `/meetings/${meeting.id}/attendance-session`, label: "Back to Attendance" }]}
      />
