  ...
  return (
    <main className="space-y-6">
      ...
      <SectionCard title="Recipients" description="Delivery results are retained for reporting and member communication history.">
        <div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="bg-slate-50 text-left text-slate-700"><tr><th className="px-4 py-3">Recipient</th><th className="px-4 py-3">Email</th><th className="px-4 py-3">Phone</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Sent</th><th className="px-4 py-3">Error</th></tr></thead><tbody>{campaign.recipients.map((recipient) => <tr key={recipient.id} className="border-t border-slate-100"><td className="px-4 py-3">{recipient.member ? formatPersonName(recipient.member) : "No member"}</td><td className="px-4 py-3">{recipient.email || "-"}</td><td className="px-4 py-3">{recipient.phone || "-"}</td><td className="px-4 py-3">{formatEnumLabel(recipient.deliveryStatus)}</td><td className="px-4 py-3">{formatDateTime(recipient.sentAt)}</td><td className="px-4 py-3">{recipient.errorMessage || "-"}</td></tr>)}</tbody></table></div>
      </SectionCard>
    </main>
  );
}
