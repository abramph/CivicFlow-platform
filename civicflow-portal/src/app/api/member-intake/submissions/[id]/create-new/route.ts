...
import { createNewMemberFromSubmission } from "@/lib/member-intake/review";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  return withApiErrorHandling(async () => {
    ...
    const result = await createNewMemberFromSubmission(organizationId, id, { userId: session.userId, userEmail: session.userEmail });
