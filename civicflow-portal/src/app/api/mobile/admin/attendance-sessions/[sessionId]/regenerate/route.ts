...
/** POST /api/mobile/admin/attendance-sessions/[sessionId]/regenerate
 * Mirrors src/app/api/attendance-sessions/[id]/regenerate/route.ts exactly --
 * bumps tokenVersion, invalidating every previously issued QR code. */
export async function POST(request: Request, { params }: { params: Promise<{ sessionId: string }> }) {
  return withApiErrorHandling(async () => {
    ...
    await createAuditEvent({
      ...
      entityId: sessionId,
      metadata: { eventId: existing.eventId, newTokenVersion: updated.tokenVersion },
    });
