...
/** POST /api/mobile/admin/hoa/properties/[propertyId]/reactivate — idempotent if already active. */
export async function POST(request: Request, { params }: RouteParams) {
  return withApiErrorHandling(async () => {
    ...
    const property = await reactivateProperty(organizationId, propertyId, { actorUserId: userId, actorEmail: email });
