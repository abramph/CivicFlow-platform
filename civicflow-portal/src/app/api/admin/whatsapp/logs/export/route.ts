...
/** GET: filterable WhatsApp log export (org, status, date range), mirroring the SMS logs export route's shape exactly. */
export async function GET(request: Request) {
  return withApiErrorHandling(async () => {
    ...
    const count = await prisma.whatsAppMessage.count({ where });
    ...
    const rows = messages.map((message) => ({
      ...
      "Message Type": message.campaignId ? "Campaign" : message.templateKey ? "Template" : "Direct",
    ...
    await createAuditEvent({
      ...
      action: "whatsapp_admin.logs_exported",
