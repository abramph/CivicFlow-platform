 ...
 * apostrophe, then applies standard CSV quoting on top. Mirrors the Meeting
 * export's csvCell — scoped to this export only. */
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  return withApiErrorHandling(async () => {
    ...
    const fileName = `unestra-attendance-${event.title.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}-${new Date().toISOString().slice(0, 10)}.csv`;
