...
- **`deterministic-generator.ts`** — the local-development fallback when `OPENAI_API_KEY` is unset. Pure keyword/pattern extraction, zero inference — every field is either genuinely found in the transcript or left null/empty. Not "a worse AI," a generator that makes zero guesses at all. Vote-outcome detection (`voteResult`) is scoped to the text window between one motion and the next (or the end of the transcript), never the whole document — a meeting with several motions with different outcomes cannot have one motion's "the motion carries" language misattributed to another motion.
- **`index.ts`**'s `resolveMeetingMinutesGenerator()` picks between them based on whether `OPENAI_API_KEY` is configured (via `config.ts`'s `getOpenAiApiKey()`).
- **The two generators use deliberately distinct disclaimer text** (`AI_GENERATED_DISCLAIMER` vs. `DETERMINISTIC_FALLBACK_DISCLAIMER`, both in `minutes/types.ts`) — the deterministic generator's output is never labeled "AI-generated," since it isn't. The Draft Minutes page (`MinutesEditor.tsx`) additionally renders a distinct visual badge ("Fallback generator — not AI" vs. "AI-generated") sourced from `MeetingMinutesDraft.generatedByProvider`, so a reviewer always knows which generator actually produced what they're reviewing without having to infer it from the prose.
...
### Upload path and verified file-size limit

Uploads are **fully proxied through the application server**, not a direct signed upload to Spaces: the client `POST`s multipart form data to `POST /api/labs/meeting-intelligence/jobs/[jobId]/upload`, which calls `request.formData()`, then `.arrayBuffer()`, then `Buffer.from(...)` — each step holds another in-memory copy of the whole file — before finally calling `uploadBufferToSpaces()`.

The enforced limit (`MAX_FILE_SIZE_BYTES`, `upload-validation.ts`, single source of truth — the upload route imports it rather than defining its own copy) is **150 MB, not 500 MB**. 500 MB was the original target, but this app's only web service (`.do/app.yaml`) is a single `apps-s-1vcpu-1gb` instance (1 GB RAM, `instance_count: 1`) that also serves every other organization's traffic — buffering a 500 MB file 2-3 times over in one request could plausibly OOM the entire portal, not just fail this one upload. 150 MB keeps peak transient memory for one upload in the ~300-450 MB range, leaving headroom for concurrent unrelated requests. The `Content-Length` header is checked first as a fast-fail (before any buffering); `validateUploadedRecording()` re-checks the real byte length independently afterward, so a spoofed/missing header cannot bypass the limit.

**Raising this back toward 500 MB requires switching to a direct client → DigitalOcean Spaces signed upload** (client `PUT`s directly to a presigned URL, bypassing the app server's memory entirely; a confirmation endpoint would then `HEAD` the object and re-run validation against a byte range read from Spaces instead of a local buffer). Not implemented in this MVP — tracked in "Deferred roadmap" below.
...
Codes: `MEETING_INTELLIGENCE_NOT_ENABLED`, `_FILE_UNSUPPORTED`, `_FILE_TOO_LARGE`, `_UPLOAD_NOT_FOUND`, `_UPLOAD_EXPIRED`, `_STORAGE_OBJECT_MISSING`, `_PROVIDER_UNAVAILABLE`, `_PROVIDER_MISCONFIGURED`, `_PROVIDER_RATE_LIMITED`, `_PROVIDER_TIMEOUT`, `_INVALID_PROVIDER_RESPONSE`, `_TRANSCRIPTION_FAILED`, `_GENERATION_FAILED`, `_JOB_NOT_FOUND`, `_JOB_CANCELLED`, `_INVALID_TRANSITION`, `_ENROLLMENT_DISABLED`, `_PERMISSION_DENIED`, `_ORGANIZATION_ARCHIVED`, `_CONSENT_REQUIRED`, `_DRAFT_NOT_EDITABLE`, `_EXPORT_NOT_APPROVED`. No route returns a generic 500 for an expected denial. `_PROVIDER_MISCONFIGURED` (operator error — missing/invalid env config) is deliberately not retryable, distinct from `_PROVIDER_UNAVAILABLE` (transient vendor outage — is retryable) — see "Environment variables" above.
...
- **Source recordings**: deleted automatically 30 days after creation (`RECORDING_RETENTION_DAYS`), via a scheduled cron job (`/api/cron/meeting-intelligence-retention`, `worker:meeting-intelligence-retention`) that only touches settled-stage jobs (`DRAFT_READY`/`IN_REVIEW`/`APPROVED`/`FAILED`/`CANCELLED`) with a storage object still present. This job has **no code path that references `MeetingMinutesDraft` at all** — it can never silently delete official minutes, by construction, not just by convention. Each job in a retention batch is processed in isolation (a per-job `try/catch`): a single Spaces failure (network error, transient outage) is logged and counted in the returned `failed` count, but does not abort the rest of the batch or block jobs ordered after it — they still get cleaned up the same tick, and the failed one retries on the next scheduled run.
- **Cascade protection at the database level**: `MeetingTranscript` and `MeetingMinutesDraft`'s foreign keys to `Organization`, `Meeting`, and `MeetingIntelligenceJob` are all `onDelete: Restrict`, not `Cascade` (see the model comments in `prisma/schema.prisma`). No feature in this codebase currently deletes an `Organization` or `Meeting` row, but if one is ever added, a `Meeting`/`Organization` delete while a transcript or minutes draft still exists for it will fail loudly with a foreign-key violation rather than silently cascading away an approved official record — the "approved snapshot is immutable by construction" claim above is enforced by the schema itself, not only by application-layer checks that a future code path could bypass. Verified against a real disposable Postgres instance: inserting an `APPROVED` draft and then attempting to delete its parent `Meeting` or `Organization` both fail with `MeetingMinutesDraft_meetingId_fkey`/`MeetingMinutesDraft_organizationId_fkey` violations.
...
## Worker deployment model, concurrency, and stale-job recovery

**Deployment model**: `worker:meeting-intelligence` and `worker:meeting-intelligence-retention` are **one-shot commands** (`tsx src/workers/*.ts` — run once, log a summary, exit), not continuous processes or a DigitalOcean "Job"/"Worker" component. There is no such component in `.do/app.yaml` — the only service defined is `web` (`instance_count: 1`). Both are also reachable as authenticated HTTP endpoints (`POST /api/cron/meeting-intelligence`, `POST /api/cron/meeting-intelligence-retention`, `Authorization: Bearer <CRON_SECRET>`), matching this repo's existing convention (`reminders`, `reports`) of relying on an **external scheduler** (recommended: cron-job.org, free tier) to `POST` on an interval — see `DEPLOYMENT.md`'s "Scheduled workers" section for exact URLs and recommended schedules (every 5 minutes for both).

**Why this matters for concurrency**: since there is no platform-level guarantee against two overlapping invocations (e.g. cron-job.org retrying a timed-out request, or a run simply taking longer than the interval), `processQueuedMeetingIntelligenceJobs()` cannot assume it has exclusive access to a `QUEUED` job just because it read it.

- **Database claim semantics**: before ever calling the transcription provider, `claimQueuedJob()` performs an atomic conditional `UPDATE` — `WHERE id = ? AND status = 'QUEUED' AND (claimedAt IS NULL OR claimedAt < staleThreshold)` — setting `claimedAt = now()`. Only one concurrent invocation can win this update (`count === 1`); the loser sees `count === 0` and skips the job for that tick without treating it as a failure. This is what actually prevents a **duplicate, billable AssemblyAI submission** for the same recording, not just a database-level race on the status column.
- **Stale-claim recovery**: a claim older than 10 minutes (`CLAIM_STALE_AFTER_MS`) on a job still `QUEUED` is treated as abandoned (the worker that claimed it crashed or was killed mid-submission, e.g. a deploy restart) and becomes reclaimable by the next tick — a job can never be permanently stuck behind a claim that will never be released.
- **Idempotency on the polling side**: `MeetingTranscript.jobId` is a unique database constraint. If two overlapping polls both observe no existing transcript and both attempt to create one, the loser's `create()` throws a unique-constraint violation (`P2002`), which is caught and treated as "another invocation already completed this" — it adopts the winning row via `findUniqueOrThrow` and proceeds normally, rather than incorrectly marking the job `FAILED` over a race it lost. (The losing invocation's already-uploaded transcript-artifact object in Spaces becomes a harmless orphan — a known, low-severity limitation, not a correctness issue.)
- **Retention failure isolation**: `runMeetingIntelligenceRetentionCleanup()` processes each job in a per-job `try/catch` — one Spaces failure doesn't block the rest of the batch (see "Retention and deletion" above).
- **Bounded retries**: see "Failure handling and retry" above — retries are explicit and bounded by the state machine, never automatic/unbounded.
- **Graceful shutdown / structured logging**: both one-shot worker scripts log a single structured summary line (`processed=`, `submitted=`/`completed=`, `failed=` counts) to stdout/stderr and exit with a non-zero code on an unhandled top-level failure — sufficient for an external scheduler's own failure-alerting (most, including cron-job.org, can alert on non-2xx/non-zero-exit). There is no in-process signal handling for graceful shutdown mid-batch; because each job is processed to completion (or claimed/failed) before the next one starts, and the claim mechanism makes a half-finished batch safely resumable by the next scheduled tick, this has not been treated as a gap requiring its own handler.
...
|---|---|---|
| `ASSEMBLYAI_API_KEY` | For real transcription | Read via `config.ts`'s `requireAssemblyAiApiKey()` at call time (not via `getServerEnv()` — same "optional, narrow, feature-specific" convention as `CRON_SECRET`, but unlike `CRON_SECRET` it is not read ad hoc — `config.ts` is the single validated entry point). Missing → throws `MEETING_INTELLIGENCE_PROVIDER_MISCONFIGURED` (500, **not retryable**) when the feature is actually used; never blocks app boot. |
| `MEETING_INTELLIGENCE_PROVIDER` | No | Overrides the default transcription provider selection (`assemblyai`), read via `config.ts`'s `resolveMeetingIntelligenceProviderId()`. An explicitly-set but unrecognized value throws `MEETING_INTELLIGENCE_PROVIDER_MISCONFIGURED` immediately rather than silently falling back to `assemblyai` — an operator typo must never resolve to a different provider than the one they intended. |
| `OPENAI_API_KEY` | For AI-generated minutes | Read via `config.ts`'s `getOpenAiApiKey()`. Missing → falls back to the deterministic (non-AI) generator automatically; never blocks app boot. |
...
**No new required environment variable was added to the strict production schema** (`src/lib/env.ts`) — all three new variables are optional, and are centralized in `src/lib/labs/meeting-intelligence/config.ts` rather than read ad hoc at each call site, so a missing credential degrades this one internal-pilot feature (with one stable, non-retryable error code) rather than breaking the whole application or leaving each call site to invent its own handling. None of the three values are ever logged, included in an error message, an audit record, or a PR/CI output — `config.ts`'s errors state only that a value is missing/invalid, never the value itself. `MEETING_INTELLIGENCE_PROVIDER_MISCONFIGURED` is deliberately excluded from `errors.ts`'s `RETRYABLE_CODES`, distinct from `MEETING_INTELLIGENCE_PROVIDER_UNAVAILABLE` (a transient vendor outage, which *is* retryable) — a misconfiguration can never be fixed by retrying, so the job-detail page's "Retry" button (driven by the `retryable` flag) is never shown for it.
...
- **Run the worker manually** (until an external cron scheduler is wired up — see "Worker deployment model" above and `DEPLOYMENT.md`): `npm run worker:meeting-intelligence` (submission + polling) and `npm run worker:meeting-intelligence-retention` (30-day recording cleanup).
...
- Upload is a **server-relay** (client → our API route → Spaces), not a presigned direct-to-Spaces upload. The enforced limit is **150 MB, verified safe for this architecture** — not the originally targeted 500 MB — because the single 1 GB-RAM web instance that serves all portal traffic also has to buffer the whole file multiple times over per upload. See "Upload path and verified file-size limit" above. Raising this requires a presigned direct-to-Spaces upload flow (tracked in "Deferred roadmap" below), not just changing a constant.
...
- Presigned direct-to-Spaces upload — required before raising the file-size limit back toward the originally targeted 500 MB (currently 150 MB, enforced by the proxied-upload architecture — see "Upload path and verified file-size limit" above).
...
5. Add the two new cron endpoints (`/api/cron/meeting-intelligence`, `/api/cron/meeting-intelligence-retention`) to the external scheduler per `DEPLOYMENT.md`'s "Scheduled workers" table (recommended: cron-job.org, every 5 minutes for both) — there is no DigitalOcean Job/Worker component to configure, only the HTTP endpoints. See "Worker deployment model, concurrency, and stale-job recovery" above.
