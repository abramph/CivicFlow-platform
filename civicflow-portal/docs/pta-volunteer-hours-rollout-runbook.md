...
- [ ] The org ID is on a documented allowlist the user has explicitly approved.
...
- [ ] The org is confirmed **not** to be an Apple or Google reviewer account. There is no automated registry of reviewer org IDs in this codebase to check against — this is a manual verification step against whatever list of reviewer-account org IDs the team maintains outside this repo (prior mobile-review-readiness work has tracked these; ask before assuming none exist). Get that list and cross-check by ID before enabling anything.
...
1. **Flip the relevant flag(s) off** — either the platform env var (kills the feature everywhere instantly) or the specific org's `PtaProfile` column(s) (kills it for just that org). This is the primary and normally sufficient rollback action.
2. **Effect is immediate**: every guard (`requireVolunteerHoursFlag`) re-checks the flag on every request — there's no cache to bust, no restart needed. The cron notification sweep (`sendVolunteerHoursNotificationsAllOrganizations`) re-filters to flag-enabled orgs on every run, so a disabled org stops receiving new notification sends on its very next scheduled run.
